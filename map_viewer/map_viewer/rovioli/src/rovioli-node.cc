...
DEFINE_string(pose_update, "", "pose generated by lidar slam.");
...
namespace rovioli {
    ...
    : is_datasource_exhausted_(false) {
        ...
        if(FLAGS_datasource_type=="rosbag"){
            datasource_flow_.reset(new DataSourceFlow(*camera_system, *maplab_imu_sensor));
        ...
        if (FLAGS_rovioli_run_map_builder || localization_enabled) {
            ...
            tracker_flow_.reset(new FeatureTrackingFlow(camera_system, *maplab_imu_sensor));
            tracker_flow_->attachToMessageFlow(flow);
            
            if (FLAGS_pose_update!="") {
                std::cout<<"pose update file: "<<FLAGS_pose_update<<std::endl;
                pose_update_flow_.reset(new PoseUpdateFlow(*camera_system, FLAGS_pose_update));
                pose_update_flow_->attachToMessageFlow(flow);
            }
        ...
        if (FLAGS_rovioli_run_map_builder) {
            map_builder_flow_.reset(new MapBuilderFlow(camera_system, std::move(maplab_imu_sensor), save_map_folder));
