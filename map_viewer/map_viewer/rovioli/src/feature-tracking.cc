...
bool FeatureTracking::trackSynchronizedNFrameImuCallback(
  ...
  //std::cout<<"inlier_matches_kp1_k: "<<inlier_matches_kp1_k[0].size()<<std::endl;
  if(FLAGS_summary_time){
  std::cout <<"chamo,"<<"extract loc features," << 1000 *(std::clock() - time) / CLOCKS_PER_SEC <<","<<synced_nframe_imu->nframe->getMinTimestampNanoseconds()/1000.0/1000.0/1000.0-1540000000<< std::endl;
  }
  previous_synced_nframe_imu_ = synced_nframe_imu;
  return true;
}
