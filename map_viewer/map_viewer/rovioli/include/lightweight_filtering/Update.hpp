...
class Update: public ModelBase<Update<Innovation,FilterState,Meas,Noise,OutlierDetection,isCoupled>,Innovation,typename FilterState::mtState,Noise>, public PropertyHandler{
 public:
  ...
  Update(): H_((int)(mtInnovation::D_),(int)(mtState::D_)),
    ...
    beta_ = 2.0;
    kappa_ = 0.0;
    updateVecNormTermination_ = 0.0001;
    maxNumIteration_  = 20;
  ...
  int performUpdate(mtFilterState& filterState, const mtMeas& meas){
    ...
    do {
      ...
      if(!isFinished){
        ...
        if(FLAGS_summary_time){
            if(mtMeas::D_==0){
                std::cout <<"chamo,"<<"update image matrix," << 1000 *(std::clock() - time1) / (float)CLOCKS_PER_SEC <<","<<filterState.t_-1540000000<< std::endl;
            }
        }
    ...
    if(FLAGS_summary_time){
        if(mtMeas::D_>2){
            std::cout <<"chamo,"<<"update pose," << 1000 *(std::clock() - time) / (float)CLOCKS_PER_SEC <<","<<filterState.t_-1540000000<< std::endl;
        }else{
            std::cout <<"chamo,"<<"update image," << 1000 *(std::clock() - time) / (float)CLOCKS_PER_SEC <<","<<filterState.t_-1540000000<< std::endl;
        }
    }
    return r;
  }
...
//         }
      ...
      if(mtMeas::D_>2){
        Eigen::Matrix3d v_cov = filterState.cov_.template block<3,3>(mtState::template getId<mtState::_vel>(),mtState::template getId<mtState::_vel>()); 
        //std::cout<<"v_cov: "<<v_cov.diagonal().transpose()<<std::endl;
      }
