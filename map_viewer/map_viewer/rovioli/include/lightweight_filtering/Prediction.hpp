...
//     std::cout<<"state.WrWM(): "<<filterState.state_.WrWM().transpose()<<std::endl;
//     std::cout<<"state.MvM(): "<<filterState.state_.MvM().transpose()<<std::endl;
//     std::cout<<"state.acb(): "<<filterState.state_.acb().transpose()<<std::endl;
//     std::cout<<"state.gyb(): "<<filterState.state_.gyb().transpose()<<std::endl;
//     std::cout<<"state.qWM(): "<<filterState.state_.qWM()<<std::endl;
//     std::cout<<"state.qCM(): "<<filterState.state_.qCM()<<std::endl;
//     std::cout<<"state.MrMC(): "<<filterState.state_.MrMC().transpose()<<std::endl;
//     Eigen::Matrix3d v_mat = filterState.cov_.template block<3,3>(mtState::template getId<mtState::_vel>(),mtState::template getId<mtState::_vel>());
//     std::cout<<v_mat(0,0)<<":"<<v_mat(1,1)<<":"<<v_mat(2,2)<<std::endl;
//     std::cout<<"=================="<<std::endl;
    ...
    if(FLAGS_summary_time){
        std::cout <<"chamo,"<<"predict," << 1000 *(std::clock() - time) / (float)CLOCKS_PER_SEC <<","<<filterState.t_-1540000000<< std::endl;
    }
    return 0;
  }
