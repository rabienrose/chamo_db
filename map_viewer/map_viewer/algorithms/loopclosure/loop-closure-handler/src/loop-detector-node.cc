...
void LoopDetectorNode::addLocalizationSummaryMapToDatabase(
  ...
  static constexpr int kDimVectors=3;
  static constexpr int kCollectTouchStatistics = 0;
  
  const Eigen::MatrixXf& index_data_ = localization_summary_map.GObserverPosition();
  index_.reset(NNSearch::createKDTreeLinearHeap(index_data_, kDimVectors, kCollectTouchStatistics));
  const Eigen::Matrix<unsigned int, Eigen::Dynamic, 1>&raw_map_data =  localization_summary_map.observerIndices();
  unsigned int max_frame_id=0;
  for(unsigned int i=0; i<raw_map_data.rows(); i++){
      if (max_frame_id<raw_map_data(i)){
          max_frame_id=raw_map_data(i);
      }
      if(frame_desc_data.count(raw_map_data(i))==0){
          std::vector<unsigned int> temp_v;
          temp_v.push_back(i);
          frame_desc_data[raw_map_data(i)]=temp_v;
      }else{
          frame_desc_data[raw_map_data(i)].push_back(i);
      }
  }
  
  const Eigen::Matrix<unsigned int, Eigen::Dynamic, 1>&raw_desc_data =  localization_summary_map.observationToLandmarkIndex();
  unsigned int max_mp_id=0;
  for(unsigned int i=0; i<raw_desc_data.rows(); i++){
      if (max_mp_id<raw_desc_data(i)){
          max_mp_id=raw_desc_data(i);
      }
      if(mp_desc_data.count(raw_desc_data(i))==0){
          std::vector<unsigned int> temp_v;
          temp_v.push_back(i);
          mp_desc_data[raw_desc_data(i)]=temp_v;
      }else{
          mp_desc_data[raw_desc_data(i)].push_back(i);
      }
  }
  localization_summary_map.getAllLandmarkIds(&mpindex_lamdmarkid);
  localization_summary_map.getAllObserverIds(&mpindex_vertexid);
  
  
  std::cout<<"mp posi count: "<<localization_summary_map.GLandmarkPosition().cols()<<std::endl;
  std::cout<<"frame count: "<<localization_summary_map.GObserverPosition().cols()<<std::endl;
  std::cout<<"mp desc count: "<<localization_summary_map.projectedDescriptors().cols()<<std::endl;
  //std::cout<<"map_frame count: "<<frame_mp_data.size()<<std::endl;
  //std::cout<<"raw_map_data count: "<<raw_map_data.rows()<<std::endl;
  //std::cout<<"max_mp_id: "<<max_mp_id<<std::endl;
...
bool LoopDetectorNode::findPoseNearby(const aslam::VisualNFrame& n_frame,
    const summary_map::LocalizationSummaryMap& localization_summary_map,
  ...
  const Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>& kp_desc_data=n_frame.getFrame(0).getDescriptors();
  loop_detector_->ProjectDescriptors(kp_desc_data, &projected_descriptors);
  ...
  double lc_knn_max_radius=5;
  const NNSearch& index_ref = *index_;
  Eigen::MatrixXi indices;
  indices.resize(num_neighbors, 1);
  Eigen::MatrixXf distances;
  distances.resize(num_neighbors, 1);
  Eigen::Vector3f query_posi;
  query_posi[0]=T_G_I->getPosition()[0];
  query_posi[1]=T_G_I->getPosition()[1];
  query_posi[2]=T_G_I->getPosition()[2];
  index_ref.knn(query_posi, indices, distances, num_neighbors, kSearchNNEpsilon, kSearchOptionsDefault,lc_knn_max_radius);
  std::cout<<"distance: "<<sqrt(distances(0))<<std::endl;
  
  const Eigen::Matrix3Xf& all_mp_posi = localization_summary_map.GLandmarkPosition();
  const Eigen::Matrix<unsigned int, Eigen::Dynamic, 1>& desc_mp_map = localization_summary_map.observationToLandmarkIndex();
  const Eigen::MatrixXf& desc_mps = localization_summary_map.projectedDescriptors();
  
  std::vector<Eigen::Vector3d> candi_mp_posi;
  std::vector<unsigned int> candi_mp_desc_id;
  std::vector<unsigned int> candi_frame_id;
  std::vector<Eigen::Vector2d> candi_mp_uv;
  //cv::Mat img= n_frame.getFrame(0).getRawImage().clone();
  //cv::cvtColor(img, img, CV_GRAY2BGR); 
  aslam::Transformation T_I_G=T_G_I->inverse();
  for (int i=0; i<indices.rows();i++){
      if(indices(i)<0){
            continue;
      }
      std::vector<unsigned int>& desc_inds=frame_desc_data[indices(i)];
      for(int j=0; j<desc_inds.size(); j++){
          unsigned int desc_id=desc_inds[j];
          unsigned int mp_id=desc_mp_map[desc_id];
          Eigen::Vector3d mp_posi;
          mp_posi[0] = all_mp_posi.col(mp_id)[0];
          mp_posi[1] = all_mp_posi.col(mp_id)[1];
          mp_posi[2] = all_mp_posi.col(mp_id)[2];
          const aslam::NCamera& cam = n_frame.getNCamera();
          //candi_mp_posi.push_back(mp_posi);
          mp_posi = T_I_G.transform(mp_posi);
          mp_posi=cam.get_T_C_B(0).transform(mp_posi);
          Eigen::Vector2d project_uv;
          aslam::ProjectionResult re_proj = cam.getCameraShared(0)->project3(mp_posi, &project_uv);
          //cv::circle(img, cv::Point(project_uv[0], project_uv[1]), 1,cv::Scalar(0, 255, 255), 2);
          if (re_proj==aslam::ProjectionResult::KEYPOINT_VISIBLE){
              candi_mp_uv.push_back(project_uv);
              candi_mp_desc_id.push_back(desc_id);
              candi_frame_id.push_back(indices(i));
          }
      }
  }
  
  if(candi_mp_uv.size()<10){
      return false;
  }

  Eigen::MatrixXd uv_egien_m;
  uv_egien_m.resize(2, candi_mp_uv.size());
  for(int i=0; i<candi_mp_uv.size(); i++){
      uv_egien_m.col(i)=candi_mp_uv[i];
  }
  
  std::shared_ptr<Nabo::NNSearchD> index_temp;
  index_temp.reset(Nabo::NNSearchD::createKDTreeLinearHeap(uv_egien_m, 2, 0));
  int num_neighbors_pt =20;
  int total_mp_match_candi=0;
  const Eigen::Matrix2Xd& frame_kp = n_frame.getFrame(0).getKeypointMeasurements();
  int matched_count=0;
  
  loop_closure::FrameToMatches frame_matches_list;
  std::vector<loop_closure::Match> match_list;
  const Eigen::Matrix<unsigned int, Eigen::Dynamic, 1>& observer_indices =localization_summary_map.observerIndices();
  
  vi_map::LandmarkIdList landmarkids;
  landmarkids.resize(frame_kp.cols());
  for(int i=0; i<landmarkids.size(); i++){
      const vi_map::LandmarkId random_landmark_id =common::createRandomId<vi_map::LandmarkId>();
      landmarkids[i]=random_landmark_id;
}
  const pose_graph::VertexId query_vertex_id(common::createRandomId<pose_graph::VertexId>());
  vi_map::VisualFrameIdentifier keyframe_id_query(query_vertex_id, 0);
  for(int i=0; i<frame_kp.cols(); i++){
        Eigen::MatrixXi indices_pt;
        indices_pt.resize(num_neighbors_pt, 1);
        Eigen::MatrixXd distances_pt;
        distances_pt.resize(num_neighbors_pt, 1);
        index_temp->knn(frame_kp.col(i), indices_pt, distances_pt, num_neighbors_pt, 1.0, NNSearch::ALLOW_SELF_MATCH, 20.0);
        const Eigen::VectorXf& projected_descriptor_pt=projected_descriptors.col(i);
        float min_dis=10000;
        unsigned int min_id=-1;
        //cv::circle(img, cv::Point(frame_kp.col(i)[0], frame_kp.col(i)[1]), 2,cv::Scalar(255, 0, 255), 3);
        for (int k=0; k<indices_pt.size(); k++){
            if(indices_pt(k)<0){
                continue;
            }
            const Eigen::VectorXf& mp_desc = desc_mps.col(candi_mp_desc_id[indices_pt(k)]);
            float diff_temp=(projected_descriptor_pt-mp_desc).norm();
        ...
        if (min_dis<3.0){
            matched_count++;
            unsigned int mp_id_matched=desc_mp_map[candi_mp_desc_id[min_id]];
            Eigen::Vector2d proj_mp=candi_mp_uv[min_id];
            //cv::line(img, cv::Point(frame_kp.col(i)[0], frame_kp.col(i)[1]), cv::Point(proj_mp[0], proj_mp[1]),cv::Scalar(255, 0, 255), 1);
            loop_closure::Match structure_match;
            structure_match.landmark_result=mpindex_lamdmarkid[mp_id_matched];
            vi_map::VisualFrameIdentifier keyframe_id_result(mpindex_vertexid[candi_frame_id[min_id]], 0);
            structure_match.keyframe_id_result = keyframe_id_result;
            structure_match.keypoint_id_query.frame_id=keyframe_id_query;
            structure_match.keypoint_id_query.keypoint_index=i;
            match_list.push_back(structure_match);
            
        }
  }
  query_vertex_observed_landmark_ids.push_back(landmarkids);
  frame_matches_list[keyframe_id_query]=match_list;
  //visualization::RVizVisualizationSink::publish("test_chamo_img", img);
  
  std::cout<<T_G_I->getPosition().transpose()<<std::endl;
  ...
  std::cout<<T_G_I->getPosition().transpose()<<std::endl;
  std::cout<<"inlier_structure_matches: "<<inlier_structure_matches->size()<<std::endl;
  *num_of_lc_matches=inlier_structure_matches->size();
  return success;
}
...
bool LoopDetectorNode::findNFrameInSummaryMapDatabase(
  ...
  if(*num_of_lc_matches>1000){
      float step=1000.0/(*num_of_lc_matches);
      int final_count=0;
      float temp_count=0;
    loop_closure::FrameToMatches frame_matches_list_t;
    for (auto const& x : frame_matches_list) {
        loop_closure::MatchVector temp_matchs;
        for(int i=0;i<x.second.size();i++){
            temp_count=temp_count+step;
            if(temp_count>1.0){
                temp_matchs.push_back(x.second[i]);
                final_count++;
                temp_count=0;
            }
        }
        if(temp_matchs.size()>0){
            frame_matches_list_t[x.first]=temp_matchs;
        }
    }
    *num_of_lc_matches=final_count;
    frame_matches_list=frame_matches_list_t;
  }
...
//   loop_closure::FrameToMatches::iterator it= frame_matches_list.begin();
//   Eigen::MatrixXf projected_descriptors;
//   const Eigen::Matrix<unsigned char, Eigen::Dynamic, Eigen::Dynamic>& kp_desc_data=n_frame.getFrame(0).getDescriptors();
...
//   for(;it!=frame_matches_list.end(); it++){
//       loop_closure::MatchVector& matches = it->second;
//       std::cout<<"frame match count: "<<matches.size()<<std::endl;
//       for(int i=0;i<matches.size(); i++){
//           vi_map::FrameKeyPointToStructureMatch& match = matches[i];
//           vi_map::LandmarkId mp_id = match.landmark_result;
//           int mp_index = localization_summary_map.getLandMarkIndex(mp_id);
//           std::vector<unsigned int>& desc_indice = mp_desc_data[mp_index];
//           unsigned int kp_index = match.keypoint_id_query.keypoint_index;
//           const Eigen::VectorXf& kp_desc = projected_descriptors.col(kp_index);
//           for(int i=0; i<desc_indice.size(); i++){
//               const Eigen::VectorXf& mp_desc= localization_summary_map.projectedDescriptors().col(desc_indice[i]);
//               float diff_temp=(kp_desc-mp_desc).norm();
//               std::cout<<it->first.vertex_id<<" | "<<mp_id<<" | "<<"diff_temp: "<<diff_temp<<std::endl;
//           }
//     }
//   }
  ...
  if(FLAGS_summary_time){
        std::cout <<"chamo,"<<"loc ransac," << 1000 *(std::clock() - time) / (float)CLOCKS_PER_SEC <<","<<n_frame.getFrame(0).getTimestampNanoseconds()/1000.0/1000.0/1000.0-1540000000<< std::endl;
    }

    
//   for (int i=0; i<inlier_structure_matches->size(); i++){
//     vi_map::VertexKeyPointToStructureMatch& match = (*inlier_structure_matches)[i];
//     vi_map::LandmarkId mp_id = match.landmark_result;
//     int mp_index = localization_summary_map.getLandMarkIndex(mp_id);
//     std::vector<unsigned int>& desc_indice = mp_desc_data[mp_index];
//     unsigned int kp_index = match.keypoint_index_query;
//     const Eigen::VectorXf& kp_desc = projected_descriptors.col(kp_index);
//     for(int i=0; i<desc_indice.size(); i++){
//         const Eigen::VectorXf& mp_desc= localization_summary_map.projectedDescriptors().col(desc_indice[i]);
//         float diff_temp=(kp_desc-mp_desc).norm();
//         std::cout<<mp_id<<" | "<<"diff_temp: "<<diff_temp<<std::endl;
//     }
//       
//   }
  *num_of_lc_matches=inlier_structure_matches->size();
...
bool LoopDetectorNode::computeAbsoluteTransformFromFrameMatches(
  ...
  for (const loop_closure::FrameIdMatchesPair& frame_matches_pair :
    ...
    if(FLAGS_summary_time){
        std::cout <<"chamo,"<<"loc project desc," << 1000 *(std::clock() - time1) / (float)CLOCKS_PER_SEC <<","<<query_vertex_n_frame.getFrame(0).getTimestampNanoseconds()/1000.0/1000.0/1000.0-1540000000<< std::endl;
    }
  int num_inliers = 0;
  double inlier_ratio = 0.0;
