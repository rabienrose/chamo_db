...
class LoopDetectorNode final
 ...
 public:
  ...
  bool findPoseNearby(const aslam::VisualNFrame& n_frame,
    const summary_map::LocalizationSummaryMap& localization_summary_map,
 ...
 private:
  ...
  std::shared_ptr<NNSearch> index_;
  std::unordered_map<unsigned int, std::vector<unsigned int> > frame_desc_data;
  std::unordered_map<unsigned int, std::vector<unsigned int> > mp_desc_data;
  vi_map::LandmarkIdList mpindex_lamdmarkid;
  pose_graph::VertexIdList mpindex_vertexid;
};
